...
class VINCOverlayView(context: Context?, attrs: AttributeSet?) : View(context, attrs) {
    ...
    override fun draw(canvas: Canvas) {
        ...
        for (result in results) {
            ...
            if (checkIsTarget(result)) {
                ...
                if (boundingBox.top < 90L) {
                    tipText = "请稍微向上"
                    Log.d("绘图层", "原始数据位置提示 请稍微向上")
                    identifyFlag = false
                }
                if (boundingBox.bottom > 210L) {
                    tipText = "请稍微向下"
                ...
                drawOneText("请走到车辆左侧雨刮器位置，找一下车架号", 300f, 260f ,canvas)
                identifyFlag = false
            }
    ...
    fun checkIsTarget(result:Detection ): Boolean {
        return "VIN" == result.categories[0].label
    }
    fun setResults(
